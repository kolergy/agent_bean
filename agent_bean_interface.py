...
iface = gr.Blocks(
    blocks=[
        gr.Block('run_action', run_action, inputs=[
            gr.Dropdown(choices=["summarize", "search"], label="Action Type"),
            gr.Textbox(lines=5, label="Action Input"),
        ]),
        gr.Block('load_document', load_document, inputs=[
            gr.Textbox(label="Document Path")
        ]),
    ],
    outputs="text",
